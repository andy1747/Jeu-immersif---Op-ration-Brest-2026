...
function renderTeam(teamId){
  ...
  Store.ensureTeam(teamId).then(()=>{
    ...
    const rerender = ()=> {
      ...
      if (currentConfig && currentConfig.finaleRevealed) renderFinaleScreen(teamId, currentConfig);
...
async function renderFinaleScreen(teamId, config){
  ...
  const potterFinal = allData.potter && allData.potter.completed && allData.potter.completed["final-potter"];
  const success = !!(potterFinal && potterFinal.status === "done");

  const html = `
    <div class="center-screen">
      <h1 style="color:var(--danger);">🏁 DOSSIER CLOS</h1>
      <p class="dim">${success
        ? "TORNADE AD SÉCURISÉE. Bon travail, agents. La Cellule Ouessant vous remercie pour votre service."
        : "Le chiffrement est devenu définitif. La formule reste hors de portée... pour cette fois."}</p>
    </div>
    <div class="card">
...
function paintTeam(teamId, team, data, config, market){
  ...
  if (!currentChapter){
    ...
    missions.forEach(mid=>{
      html += renderMissionCard(teamId, mid, completed, false, config);
    });
  }
  ...
  if (data.minuitFinalUnlocked){
    ...
    const heading = (finalDef && finalDef.type === "omega") ? "⚠️ Protocole Omega" : "⚠️ Mission finale";
    html += `<h2 style="margin-top:22px;">${heading}</h2>`;
    html += renderMissionCard(teamId, finalId, completed, true, config, data.omegaActivatedAt);
  }
  ...
  if (data.minuitFinalUnlocked) wireMissionCard(teamId, "final-"+teamId, completed, blocked, true, data.omegaActivatedAt);
...
function renderMissionCard(teamId, missionId, completed, isFinal, config, omegaActivatedAt){
  const def = getMissionDef(missionId);
  ...
  const eyebrow = isOmega ? "⚠️ PROTOCOLE OMEGA" : isFinal ? "⚠️ PRIORITÉ MAXIMALE" : (isChoix ? "🎯 ORDRE DE MISSION" : "📂 DOSSIER CONFIDENTIEL");
