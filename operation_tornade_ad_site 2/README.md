...
Enquête**, **3. Protocole Omega**. Chaque activation envoie une vraie
notification push à toutes les équipes.

**Deux équipes sont secrètement des agents dormants** (Casa de Papel et Harry
Potter), chacune ignorant l'existence de l'autre — voir section 8. **Harry
Potter** est aussi la cellule qui vivra le final : retrouver Roland et saisir
le CODE OMEGA avant la fin du compte à rebours (section 9).
...
- `teams` : nom, objectif, pouvoir, thème visuel, un champ `missions` qui
  ...
  (`{ 1: ["roland"], 2: [...4 missions...], 3: [...1 mission climax...] }`),
  un champ `finalMission` (uniquement rempli pour `potter`, qui porte le
  Protocole Omega), et pour les deux équipes infiltrées : `dormant: true` +
  `handler` (nom du commanditaire).
...
- `missions` : le catalogue des 25 missions principales (5 par équipe),
  courtes, comprises en moins de 10 secondes, validées d'un seul tap. Une
  mission par équipe au chapitre 2 est de type `"choix"` (risque vs
  sécurité, résolution instantanée).
...
- `finalMissions` : uniquement `final-potter` — le **Protocole Omega**
  (`type: "omega"`, `dureeMinutes`, `omegaCode`). Débloqué à l'activation du
  Chapitre 3, avec une interface dédiée (compte à rebours + saisie de code)
  au lieu du circuit preuve/validation classique — voir section 9.
...
3. **Protocole Omega** — la mission climax de chaque équipe se débloque, et
   Harry Potter reçoit en plus sa notification secrète : un compte à rebours
   et un champ de saisie de code apparaissent dans son espace (voir section 9).
   Les pouvoirs et protections sont réinitialisés pour tout le monde.
...
sont crédités instantanément. Seul le **Protocole Omega** (Harry Potter) a
sa propre interface (compte à rebours + code à 4 chiffres), décrite en
section 9.
...
**Fin de soirée** : depuis l'onglet **🕶️ Dormants** (voir section 8), tu
déclenches la révélation finale — classement, réussite/échec du Protocole
Omega, et démasquage des deux agents dormants — puis l'onglet **🥇
Récompenses** pour annoncer les prix (équipe gagnante, meilleure enquête,
meilleur bluff, mission la plus drôle, meilleur agent double, meilleure
preuve).
...
de chaque équipe infiltrée, rappel du CODE OMEGA à donner en personne via
« Roland », et le bouton **🏁 Déclencher la révélation finale**, qui envoie
à toutes les équipes en même temps : le classement officiel, la réussite ou
l'échec du Protocole Omega, et le démasquage des deux agents dormants (avec
...
## 9. Le Protocole Omega (final)

Au Chapitre 3, Harry Potter reçoit une notification secrète et un bloc
dédié apparaît dans son espace, à la place du circuit preuve/validation
habituel :

- un **compte à rebours** (12 minutes par défaut, `dureeMinutes` dans
  `finalMissions["final-potter"]`) ;
- un champ de saisie pour le **CODE OMEGA** (4 chiffres, `omegaCode` dans
  `data.js` — visible pour toi dans l'onglet 🕶️ Dormants) ;
- un bouton « 🔓 Déchiffrer » qui vérifie le code en direct (`Store.
  validateOmegaCode`). Bon code → mission créditée, tout le monde peut être
  averti par la suite via la révélation finale. Mauvais code → message
  d'erreur, nombre d'essais illimité (la tension vient du chrono, pas de la
  frustration).

Le code est censé être obtenu **en interrogeant Roland en personne** avant
son départ (chapitre 1) ou en le retrouvant physiquement pendant le compte
à rebours — improvise cette partie en direct selon la qualité de vos
échanges avec les équipes, exactement comme la fenêtre Roland elle-même.
...
Protocole Omega). Un bouton rond 🔊/🔇 flottant en bas à droite de chaque
...
« grésiller », police terminal pour le compte à rebours du Protocole Omega.
...
Briefing & Roland, Enquête ou Protocole Omega — voir section 7), **🏆
Scores** (scores en direct, ajustement manuel des points, bloquer/débloquer
une équipe — affiche aussi les points Handler des agents dormants), **📸
Preuves** (valider ou refuser le Protocole Omega si besoin), **🎲 Tirages**
(rappel sur la fenêtre Roland + indices/rebondissements libres), **🌩️
Événements**, **🧭 Demandes** (indice du pouvoir des Aventuriers), **🔔
Notifications** (actions rapides + brouillons — voir section 4.6), **🕶️
Dormants** (confidentiel — voir section 8), et **🥇 Récompenses**
(classement final).
...
- [ ] Tester l'activation des 3 chapitres depuis l'onglet **📖 Chapitres**,
      vérifier le Canal privé des deux agents dormants, et tester le
      Protocole Omega (mauvais code puis bon code : `2920` par défaut,
      modifiable dans `data.js` → `finalMissions["final-potter"].omegaCode`)
