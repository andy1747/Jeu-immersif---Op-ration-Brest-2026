...
function defaultTeam(){
  return { score:0, unlockedCount:1, completed:{}, powerUsed:false, powerUsedAt:null,
           ...
           drawnChallenges: {}, festifsDone: {}, introSeen:false };
}
...
function coreMissionIds(teamId, currentChapter, teamData){
  ...
  // Défis festifs : obligatoires et notés dès le chapitre 2 (même liste pour
  // toutes les équipes, pas de tirage aléatoire) — comptent donc désormais
  // dans "l'équipe a terminé toutes ses missions" (voir teamMissionsComplete).
  if (chapterNum >= 2 && GAME_DATA.challengeBank && GAME_DATA.challengeBank.festifs){
    out = out.concat(GAME_DATA.challengeBank.festifs.map(f => f.id));
  }
  return out;
}
