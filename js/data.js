...
const GAME_DATA = {
  ...
  teams: {
    casa: {
      ...
      missions: { 1: ["roland"], 2: { draw: 4 }, 3: ["quiz-ipc"] },
      finalMission: null,
  ...
  // pour reconstituer ce code (voir js/store.js: validateCoffreCode).
  // ---------------------------------------------------------------
  coffreCode: "TORNADEIPC",
  ...
  // ---------------------------------------------------------------
  // MISSIONS COMMUNES HORS-CHAPITRE-2 — Roland (chapitre 1) et Quiz IPC
  // (chapitre 3). Les 4 défis du chapitre 2 ne sont plus fixes : ils sont
  // tirés au sort par équipe dans challengeBank ci-dessous (voir
  // teams.*.missions[2] = { draw: 4 } + Store.drawChallenges).
  // ---------------------------------------------------------------
  missions: {
  ...
  challengeBank: {
    ...
    // ---------------------------------------------------------------
    // 🥂 DÉFIS FESTIFS — facultatifs, jamais obligatoires, jamais tirés
    // au sort avec les autres (pas de pénalité si ignorés). Affichés en
    // permanence dès le chapitre 2 dans un bloc à part, avec un bouton
    // "Fait !" ET un bouton "Passer" équivalents (aucun impact sur le
    // score). Volontairement aucun défi type "cul-sec" scripté par
    // l'app : à l'organisatrice/aux équipes de gérer ça de vive voix si
    // elles le souhaitent, dans le respect du consentement de chacun.
    // ---------------------------------------------------------------
    festifs: [
      { id:"fest-01", titre:"Le Toast", description:"Portez un toast improvisé au nom d'un produit IPC." },
      { id:"fest-02", titre:"Le Cocktail IPC", description:"Inventez le nom d'un cocktail (ou mocktail) inspiré d'un produit IPC — pas besoin de le préparer !" },
      { id:"fest-03", titre:"Trinquons", description:"Allez trinquer avec une autre équipe, à votre santé à tous." }
    ]
  },
