...
/* ---------------- CINÉMATIQUE D'INTRODUCTION "DOSSIER CONFIDENTIEL IPC" ----------------
   Jouée une seule fois par équipe (état partagé Firestore/localStorage, voir
   Store.markIntroSeen — jamais par appareil) : écran de chargement "Connexion
   au serveur sécurisé IPC..." puis terminal à effet machine à écrire. Bouton
   "Passer l'introduction" disponible à tout moment ; taper sur le texte du
   terminal termine instantanément la frappe en cours. */
const INTRO_BRIEFING_LINES = [
  "🚨 DOSSIER CONFIDENTIEL IPC",
  "Niveau de confidentialité : MAXIMUM",
  "",
  "Bienvenue Agents.",
  "",
  "Cette nuit, un événement exceptionnel vient de se produire : le dossier ultra confidentiel concernant Tornade AD a disparu des archives d'IPC.",
  "",
  "Votre mission : retrouver les indices, réussir toutes les épreuves de votre cellule, et obtenir votre chiffre secret.",
  "",
  "Aucune équipe ne pourra ouvrir le Coffre IPC seule. Vous devrez vous réunir en fin de soirée, tous ensemble, autour du téléphone principal, pour reconstituer le code complet.",
  ""
];
function playIntroCinematic(team, onDone){
  ...
  function showLoading(){
    overlay.innerHTML = `
      <button class="btn-outline btn-sm intro-skip-btn" id="intro-skip-btn">Passer l'introduction</button>
      <div class="intro-loading">
        <div class="cine-line terminal-text">🔒 Connexion au serveur sécurisé IPC...</div>
        <div class="loading-bar"><div class="loading-bar-fill"></div></div>
      </div>
    `;
    $("intro-skip-btn").onclick = finish;
    loadingTimer = setTimeout(showTerminal, 2400);
  }

  function showTerminal(){
    const fullText = INTRO_BRIEFING_LINES.concat([`Cellule activée : ${team.nom}.`]).join("\n");
    overlay.innerHTML = `
      <button class="btn-outline btn-sm intro-skip-btn" id="intro-skip-btn">Passer l'introduction</button>
      <div class="terminal-screen" id="intro-terminal-screen">
        <pre class="terminal-typed" id="intro-terminal-text"></pre>
      </div>
      <button class="btn-block intro-start-btn" id="intro-start-btn" style="display:none;">▶ Commencer la mission</button>
    `;
    $("intro-skip-btn").onclick = finish;
    const textEl = $("intro-terminal-text");
    const revealAll = ()=>{
      if (typeInterval) clearInterval(typeInterval);
    ...
    function showStartButton(){
      ...
      if (startBtn){ startBtn.style.display = "block"; startBtn.onclick = finish; }
    }
    let idx = 0;
    ...
    $("intro-terminal-screen").onclick = revealAll;
  }
...
// quand la Coffre IPC vient d'être ouverte (code validé par n'importe
// quelle équipe). Même mécanique que playChapterIntro (overlay auto-fermant).
function playCoffreOpeningCinematic(team, onDone){
  ...
  const steps = [
    ...
    { html: `<div class="coffre-open-anim">🔐</div><div class="cine-title" style="font-size:1.5rem;">LE COFFRE S'OUVRE</div>`, wait: 2200 },
...
let _lastAllTeams = {}; // dernier snapshot de toutes les équipes (pour la condition "5 quiz terminés" de la Coffre IPC)
...
function renderTeam(teamId){
  ...
  // ⚠️ La musique (suspense d'intro ou ambiance normale) ne peut démarrer
  // qu'une fois qu'on sait si la cinématique d'intro doit jouer — décidé plus
  // bas, dans rerender(), dès que les données d'équipe sont connues.
  let audioStarter = null;
  const ambientUnlock = ()=> { if (audioStarter){ try{ audioStarter(); }catch(e){} } };
    ...
    const rerender = ()=> {
      ...
      if (currentConfig && currentConfig.finaleRevealed){
        if (!_finaleCineFired[teamId]){
          ...
          playCoffreOpeningCinematic(team, ()=> renderFinaleScreen(teamId, currentConfig));
        } else {
      ...
      if (!introDecided){
        introDecided = true;
        if (!currentTeamData.introSeen){
          introPlaying = true;
          audioStarter = ()=>{ if (window.AudioFX){ AudioFX.ensureCtx(); AudioFX.startIntroSting(); } };
          audioStarter();
          playIntroCinematic(team, ()=>{
            introPlaying = false;
            if (window.AudioFX) AudioFX.stopIntroSting();
            audioStarter = ()=> engageAmbient(team.theme);
            engageAmbient(team.theme);
            Store.markIntroSeen(teamId);
            paintTeam(teamId, team, currentTeamData, currentConfig, currentMarket);
          });
          return;
        }
        audioStarter = ()=> engageAmbient(team.theme);
...
// Certains chapitres (ex: chapitre 2) ne sont plus une liste fixe mais un
// tirage aléatoire par équipe ({draw:N} dans data.js) : dans ce cas, on lit
// les identifiants réellement tirés dans teamData.drawnChallenges[c]
// (posé une seule fois par Store.activateChapter, jamais retiré ensuite).
function missionListFor(teamId, currentChapter, teamData){
...
// Défis festifs tirés au sort pour cette équipe (banque commune, voir
// Store.selectFestifsDraw) — jamais toute la banque, jamais forcément le
// même tirage qu'une autre équipe. Même structure que missionListFor.
function drawnFestifsFor(teamId, currentChapter, teamData){
  const chapterNum = currentChapter || 0;
  const byChapter = (GAME_DATA.teams[teamId] || {}).missions || {};
  let out = [];
  for (let c = 1; c <= chapterNum; c++){
    ...
    if (slot && typeof slot === "object" && slot.draw){
      const drawn = (teamData && teamData.drawnFestifs && teamData.drawnFestifs[c]) || [];
      out = out.concat(drawn);
    }
  }
  return out;
...
async function renderFinaleScreen(teamId, config){
  ...
  const success = !!(config && config.coffreOpened);

  const bestExpertScore = Math.max(0, ...TEAM_ORDER.map(t=>(allData[t]||{}).ipcQuiz && allData[t].ipcQuiz.correct || 0));
  const bestExperts = TEAM_ORDER.filter(t=> ((allData[t]||{}).ipcQuiz||{}).correct === bestExpertScore && bestExpertScore>0);
  const speeds = TEAM_ORDER
    .map(t=>({ t, ts: ((allData[t]||{}).completed||{})["quiz-ipc"] ? (allData[t].completed["quiz-ipc"].timestamp||Infinity) : Infinity }))
    .filter(x=>x.ts !== Infinity).sort((a,b)=>a.ts-b.ts);
  const fastestTeam = speeds[0] && speeds[0].t;

  const html = `
    <div class="center-screen">
      <h1 style="color:var(--success);">🎉 MISSION ACCOMPLIE !</h1>
      <p class="dim">${success
        ? "Le Coffre IPC est ouvert. Félicitations à tous les agents !"
        : "La révélation a été déclenchée manuellement par l'organisatrice, avant l'ouverture complète du coffre."}</p>
    </div>
    <div class="card">
      <h3>🏆 Classement final</h3>
      <ol style="padding-left:20px;">
        ${ranking.map(r=>{
          const done = Object.values((allData[r.t]||{}).completed||{}).filter(c=>c.status==="done").length;
          return `<li>${GAME_DATA.teams[r.t].nom} — <b>${r.score} pts</b> · ${done} défis réussis</li>`;
        }).join("")}
      </ol>
    </div>
    <div class="card" style="border-color:var(--accent);">
      ...
      <ul class="indice-list">
        ${bestExperts.length ? `<li>🧠 Meilleur(s) expert(s) IPC : <b>${bestExperts.map(t=>GAME_DATA.teams[t].nom).join(", ")}</b> (${bestExpertScore}/5 au Quiz IPC)</li>` : ""}
        ${fastestTeam ? `<li>⚡ Équipe la plus rapide au Quiz IPC : <b>${GAME_DATA.teams[fastestTeam].nom}</b></li>` : ""}
        <li>🎉 Meilleure ambiance : à annoncer en direct par l'organisatrice (onglet 🥇 Récompenses)</li>
      </ul>
    </div>
    <div class="card">
      ...
      <ul class="indice-list">
        ${TEAM_ORDER.map(t=>{
          ...
          return `<li><b>${GAME_DATA.teams[t].nom}</b> — ${q.correct||0} bonnes réponses, ${q.wrong||0} mauvaises, chiffre secret « ${GAME_DATA.teams[t].codeDigit} »</li>`;
        }).join("")}
      </ul>
    </div>
...
function getMissionDef(missionId){
  return GAME_DATA.commonMissions[missionId] || GAME_DATA.missions[missionId]
    ...
    || (GAME_DATA.challengeBank && GAME_DATA.challengeBank.items.find(i => i.id === missionId));
}
...
function paintTeam(teamId, team, data, config, market){
  ...
  const missions = missionListFor(teamId, currentChapter, data);
  ...
  // Les défis festifs sont obligatoires et notés dès le chapitre 2 : ils
  // comptent donc dans le total de missions. Banque commune, mais tirage
  // propre à cette équipe (voir Store.selectFestifsDraw) — jamais la liste
  // entière, jamais le même tirage qu'une autre équipe.
  const drawnFestifIds = drawnFestifsFor(teamId, currentChapter, data);
  const totalMissions = missions.length + drawnFestifIds.length + (data.minuitFinalUnlocked ? 1 : 0);
  ...
  html += `
    <div class="team-header">
      ...
      <span class="badge">Équipe ${Store.teamNumber(teamId)}</span>
      ...
      <button class="btn-outline btn-sm" id="replay-intro-btn" style="margin-top:10px;">🔁 Revoir l'introduction</button>
    </div>
  `;

  // 🔑 Chiffre secret — n'apparaît QUE lorsque l'équipe a terminé
  // ABSOLUMENT toutes ses missions en cours (voir Store.teamMissionsComplete).
  // Aucune équipe ne voit jamais le chiffre d'une autre équipe, et le code
  // complet ne se saisit que sur le téléphone principal (admin), jamais ici.
  if (Store.teamMissionsComplete(teamId, currentChapter, data)){
    html += `<div class="card pulse" style="border-color:var(--accent); text-align:center;">
      <span class="badge dossier-tag">🔑 Toutes les missions accomplies</span>
      <h3>Votre chiffre secret</h3>
      <div class="score-display" style="font-size:2.4rem; letter-spacing:6px;">${team.codeDigit}</div>
      <p class="dim">Retenez-le précieusement — vous seuls le connaissez. Rejoignez le téléphone principal en fin de soirée pour reconstituer le code complet avec les 4 autres équipes.</p>
    </div>`;
  }
  ...
  // 🥂 Défis festifs — obligatoires et notés au même titre que les autres
  // défis, dès le chapitre 2. Banque commune, mais tirage propre à chaque
  // équipe (drawnFestifIds, voir plus haut) : jamais toute la banque, jamais
  // le même tirage qu'une autre équipe. Purs moments d'ambiance de
  // séminaire : jamais de gestion d'alcool ni de "cul sec" côté appli,
  // chacun trinque avec ce qu'il veut (voir data.js pour la consigne complète).
  if (drawnFestifIds.length){
    const festifsDone = data.festifsDone || {};
    html += `<h2 style="margin-top:22px;">🥂 Défis festifs</h2>
      <p class="dim">Ambiance de séminaire, à faire vivre en équipe — notés comme les autres défis. Libre à chacun de trinquer avec ce qu'il préfère.</p>`;
    drawnFestifIds.map(id => GAME_DATA.challengeBank.festifs.find(f=>f.id===id)).filter(Boolean).forEach(f=>{
      const st = festifsDone[f.id];
      const done = st && st.status === "done";
      html += `<div class="card pop-in dossier">
        <span class="badge dossier-tag">🥂 FESTIF</span>
        <span class="badge points">+${f.points} pts</span>
        ${done ? `<span class="badge done">✅ Fait !</span>` : ""}
        <h3>${f.titre}</h3>
        <p>${f.description}</p>
        ${!done ? `<button class="btn-block btn-sm" data-festif-done="${f.id}">✔️ Fait !</button>` : ""}
      </div>`;
    });
  }
  ...
  // Coffre IPC), mais une équipe testée AVANT ce remplacement peut avoir
  ...
  // Le Coffre IPC — finale commune. ⚠️ Le code ne se saisit JAMAIS sur le
  // téléphone d'une équipe : c'est volontaire (cf. spec Andreia). Une fois
  // que TOUTES les équipes ont terminé TOUTES leurs missions, chaque
  // téléphone affiche simplement une invitation à rejoindre le téléphone
  // principal (celui de l'organisatrice, dans admin.html), qui est le seul
  // endroit où le clavier du Coffre IPC existe.
  const allTeamsData = _lastAllTeams || {};
  const everyoneDone = Store.allTeamsComplete(config && config.currentChapter, allTeamsData);
  if (everyoneDone && !(config && config.coffreOpened)){
    html += `
      <div class="card pulse" style="border-color:var(--accent); text-align:center; margin-top:22px;">
        ...
        <p class="dim">Toutes les équipes ont accompli leurs missions. Veuillez rejoindre le téléphone principal pour la phase finale de la mission — le Coffre IPC n'attend plus que votre chiffre secret.</p>
      </div>`;
  }
  ...
  const replayIntroBtn = $("replay-intro-btn");
  if (replayIntroBtn) replayIntroBtn.onclick = ()=>{
    // Simple relecture manuelle : ne touche jamais à introSeen (qui reste
    // "vu" pour l'équipe), n'affecte donc pas les autres appareils/rechargements.
    if (window.AudioFX){ AudioFX.ensureCtx(); AudioFX.startIntroSting(); }
    playIntroCinematic(team, ()=>{
      if (window.AudioFX) AudioFX.stopIntroSting();
      engageAmbient(team.theme);
      paintTeam(teamId, team, data, config, market);
    });
  };
  ...
  document.querySelectorAll("[data-festif-done]").forEach(b=> b.onclick = async ()=>{
    b.disabled = true;
    const res = await Store.markFestif(teamId, b.dataset.festifDone);
    if (window.AudioFX){ try{ AudioFX.play("validation", team.theme); }catch(e){} }
    toast("🥂 Bien joué !", res && !res.already ? `+${res.points} points` : "", "success");
  });

  // ⚠️ Aucun input/bouton de saisie du Coffre IPC ici : volontairement absent
  // des téléphones d'équipe (voir bloc "everyoneDone" ci-dessus). Le clavier
  // du Coffre IPC n'existe que sur le téléphone principal (admin.html).
...
function renderMissionCard(teamId, missionId, completed, isFinal, config, omegaActivatedAt, teamData){
  ...
  const isQuiz1 = def.kind === "quiz1";
  const isRoue = def.kind === "roue";

  // Les défis quiz1 (produits/culture) n'ont pas de champ "description" : le
  // texte à afficher est la question elle-même, sinon la carte reste muette
  // avant réponse (bug corrigé : affichait littéralement "undefined").
  let desc = def.description || (isQuiz1 ? def.question : "");
  ...
  if (isQuiz1 && state && state.status === "done"){
    cardClass += " done";
    return `<div class="${cardClass}" id="mission-${missionId}">
      <span class="badge dossier-tag">🧴 DÉFI PRODUIT</span>
      ${state.correct
        ? `<span class="badge done">✅ Bonne réponse (+${state.points} pts)</span>`
        : `<span class="badge" style="background:var(--danger);color:#fff;">❌ Mauvaise réponse (0 pt)</span>`}
      <h3>${def.titre}</h3>
      <p>${def.question}</p>
      ${def.explanation ? `<p class="dim">${def.explanation}</p>` : ""}
    </div>`;
  }
  if (isRoue && state && state.status === "done"){
    cardClass += " done";
    return `<div class="${cardClass}" id="mission-${missionId}">
      <span class="badge dossier-tag">🎡 MINI-JEU</span>
      <span class="badge done">🎉 ${state.outcome}</span>
      <h3>${def.titre}</h3>
    </div>`;
  }
  ...
  if (isQuiz && state && state.status === "done"){
    ...
    const digit = (GAME_DATA.teams[teamId]||{}).codeDigit;
    const allDone = Store.teamMissionsComplete(teamId, config.currentChapter, teamData);
    return `<div class="${cardClass}" id="mission-${missionId}">
      ...
      <div class="card" style="border-color:var(--accent); text-align:center; margin-top:10px;">
        <p class="dim">🔑 Votre chiffre secret</p>
        <div class="score-display" style="font-size:1.6rem; letter-spacing:4px;">${digit}</div>
        <p class="dim">Rejoignez le téléphone principal en fin de soirée.</p>
      </div>` : `
      <p class="dim" style="margin-top:8px;">🔒 Terminez toutes vos missions pour débloquer votre chiffre secret.</p>`}
    </div>`;
  }
  ...
  if (state && state.status === "done"){
    ...
    bodyExtra = `<div id="quiz1-options-${missionId}">
      ${def.options.map((opt,i)=>`<button class="btn-block btn-outline quiz-option" style="margin-bottom:8px;" data-quiz1="${missionId}" data-opt="${i}">${opt}</button>`).join("")}
    </div>`;
  } else if (isRoue){
    const n = def.outcomes.length;
    const colors = ["#9d6bff","#ff6bd6","#38d996","#ff5a5f","#ffd166","#4dd6e8"];
    const slice = 360/n;
    const gradient = def.outcomes.map((o,i)=> `${colors[i%colors.length]} ${i*slice}deg ${(i+1)*slice}deg`).join(", ");
    bodyExtra = `<div class="roue-wrap">
        <div class="roue-disque" id="roue-${missionId}" style="background: conic-gradient(${gradient});"></div>
        <div class="roue-pointeur">▼</div>
      </div>
      <button class="btn-block" data-roue="${missionId}" style="margin-top:10px;">🎡 Lancer la roue</button>
      <p class="dim" id="roue-status-${missionId}" style="margin-top:6px;"></p>`;
  } else if (isFinal){
  ...
  const eyebrow = isOmega ? "⚠️ PROTOCOLE OMEGA" : isQuiz ? "🧠 QUIZ IPC" : isQuiz1 ? "🧴 DÉFI PRODUIT" : isRoue ? "🎡 MINI-JEU" : isFinal ? "⚠️ PRIORITÉ MAXIMALE" : (isChoix ? "🎯 ORDRE DE MISSION" : "📂 DOSSIER CONFIDENTIEL");

  return `<div class="${cardClass}" id="mission-${missionId}">
    <span class="badge dossier-tag">${eyebrow}</span>
    ${pointsBadge} ${statusBadge}
    <h3>${def.titre}</h3>
    ${def.image ? `<img src="${def.image}" alt="${def.titre}" class="challenge-image">` : ""}
    ${desc ? `<p>${desc}</p>` : ""}
...
function wireMissionCard(teamId, missionId, completed, blocked, isFinal, omegaActivatedAt){
  ...
  document.querySelectorAll(`[data-quiz1="${missionId}"]`).forEach(b=>{
    b.disabled = !!blocked;
    b.onclick = ()=> answerQuiz1(teamId, missionId, parseInt(b.dataset.opt,10));
  });

  const roueBtn = document.querySelector(`[data-roue="${missionId}"]`);
  if (roueBtn){
    roueBtn.disabled = !!blocked;
    roueBtn.onclick = ()=> spinRoue(teamId, missionId);
  }
}

/* ---------------- DÉFI PRODUIT (quiz1 — une seule question) ---------------- */
async function answerQuiz1(teamId, missionId, selectedIndex){
  document.querySelectorAll(`[data-quiz1="${missionId}"]`).forEach(b=> b.disabled = true);
  const res = await Store.answerSingleQuiz(teamId, missionId, selectedIndex);
  if (!res.ok) return;
  const team = GAME_DATA.teams[teamId];
  if (res.correct){
    toast("✅ Bonne réponse", `+${res.points||0} points`, "success");
    if (window.AudioFX){ try{ AudioFX.play("validation", team.theme); }catch(e){} }
  } else {
    toast("❌ Mauvaise réponse", res.explanation||"", "fail");
    if (window.AudioFX){ try{ AudioFX.play("event", team.theme); }catch(e){} }
  }
}

/* ---------------- ROUE DU HASARD ---------------- */
async function spinRoue(teamId, missionId){
  const def = getMissionDef(missionId);
  const btn = document.querySelector(`[data-roue="${missionId}"]`);
  const disque = $("roue-"+missionId);
  const status = $("roue-status-"+missionId);
  if (btn) btn.disabled = true;
  const res = await Store.spinWheel(teamId, missionId, def);
  const n = def.outcomes.length;
  const slice = 360 / n;
  // Pointe vers le milieu de la tranche gagnante, + plusieurs tours complets pour l'effet.
  const targetAngle = 360*4 + (360 - (res.outcomeIndex*slice + slice/2));
  if (disque){
    disque.style.transition = "transform 3.2s cubic-bezier(.17,.67,.24,1)";
    disque.style.transform = `rotate(${targetAngle}deg)`;
  }
  const team = GAME_DATA.teams[teamId];
  setTimeout(()=>{
    if (status) status.textContent = res.outcome;
    if (window.AudioFX){ try{ AudioFX.play(res.points>0?"victory":"event", team.theme); }catch(e){} }
    toast("🎡 Roue du Hasard", res.outcome, res.points>=0?"success":"fail");
    setTimeout(()=> renderTeam(teamId), 1400);
  }, 3300);
}
...
function renderQuizQuestion(teamId, missionId, questions, idx, stats){
  ...
  // La toute première question construit l'écran une fois ; les suivantes
  // ne remplacent QUE le contenu de la carte (question/options/feedback),
  // sans jamais recréer $("app").innerHTML — ça évite que la page saute en
  // haut et que la carte "bouge" à chaque nouvelle question.
  if (!$("quiz-shell")){
    $("app").innerHTML = `
      <div class="team-header" style="padding-bottom:0;" id="quiz-shell">
        <span class="badge" id="quiz-progress-label"></span>
        <span class="badge points" id="quiz-score-label"></span>
      </div>
      <div class="progress-wrap"><div class="progress-bar" id="quiz-progress-bar" style="width:0%"></div></div>
    ...
    $("quiz-card").classList.remove("quiz-flash-correct","quiz-flash-wrong");
  }
  $("quiz-progress-label").textContent = `🧠 Question ${idx+1}/${total}`;
  $("quiz-score-label").textContent = `Score : ${stats.score||0} pts`;
  $("quiz-progress-bar").style.width = pct+"%";
  $("quiz-question").textContent = q.question;
  $("quiz-options").innerHTML = q.options.map((opt,i)=>`<button class="btn-block btn-outline quiz-option" data-opt="${i}">${opt}</button>`).join("");
  $("quiz-feedback").innerHTML = "";
  $("quiz-timer").textContent = GAME_DATA.ipcQuiz.timerSeconds;
  $("quiz-timer").style.color = "";
...
async function finishIpcQuiz(teamId, missionId, stats){
  ...
  const team = GAME_DATA.teams[teamId];
  // Le chiffre secret ne se révèle que si TOUTES les missions de l'équipe
  // sont désormais terminées (pas seulement le quiz) — voir Store.teamMissionsComplete.
  const freshData = await Store.getTeam(teamId);
  const currentChapter = _lastConfig.currentChapter || 0;
  const allDone = Store.teamMissionsComplete(teamId, currentChapter, freshData);
  $("app").innerHTML = `
    <div class="center-screen">
    ...
    <div class="card" style="border-color:var(--accent); text-align:center;">
      <span class="badge dossier-tag">🔑 Toutes les missions accomplies</span>
      <p class="dim">Votre chiffre secret</p>
      <div class="score-display" style="font-size:2rem; letter-spacing:6px;">${team.codeDigit}</div>
      <p class="dim">Retenez-le précieusement. Rejoignez le téléphone principal en fin de soirée pour reconstituer le code complet.</p>
    </div>` : `
    <div class="card" style="text-align:center;">
      <p class="dim">Il vous reste encore des missions à accomplir avant de recevoir votre chiffre secret. Continuez l'enquête !</p>
    </div>`}
  ...
  if (type === "vol_points"){
    ...
    others.forEach(t=>{
      document.querySelector(`[data-target="${t}"]`).onclick = async ()=>{
        ...
        const otherMissions = missionListFor(t, chapterNow, otherData).filter(mid=> !otherData.completed || !otherData.completed[mid]);
        const myMissions = missionListFor(teamId, chapterNow, myData).filter(mid=> !myData.completed || !myData.completed[mid]);
