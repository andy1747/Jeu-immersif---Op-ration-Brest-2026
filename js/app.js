...
function paintTeam(teamId, team, data, config, market){
  ...
  // comptent donc dans le total de missions (même liste pour toutes les
  // équipes, cf. GAME_DATA.challengeBank.festifs).
  const festifsCount = (currentChapter >= 2 && GAME_DATA.challengeBank && GAME_DATA.challengeBank.festifs) ? GAME_DATA.challengeBank.festifs.length : 0;
  const totalMissions = missions.length + festifsCount + (data.minuitFinalUnlocked ? 1 : 0);
  ...
  // défis, dès le chapitre 2. Purs moments d'ambiance de séminaire : jamais
  // de gestion d'alcool ni de "cul sec" côté appli, chacun trinque avec ce
  // qu'il veut (voir data.js pour la consigne complète).
  if (currentChapter >= 2 && GAME_DATA.challengeBank && GAME_DATA.challengeBank.festifs.length){
    ...
    GAME_DATA.challengeBank.festifs.forEach(f=>{
