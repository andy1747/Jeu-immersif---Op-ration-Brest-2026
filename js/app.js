...
function paintTeam(teamId, team, data, config, market){
  ...
  // 🥂 Défis festifs — facultatifs, jamais obligatoires, jamais notés.
  // Toujours visibles dès le chapitre 2, indépendamment du tirage aléatoire.
  if (currentChapter >= 2 && GAME_DATA.challengeBank && GAME_DATA.challengeBank.festifs.length){
    const festifsDone = data.festifsDone || {};
    html += `<h2 style="margin-top:22px;">🥂 Défis festifs (facultatifs)</h2>
      <p class="dim">Purement pour l'ambiance — aucun point en jeu, aucune obligation.</p>`;
    GAME_DATA.challengeBank.festifs.forEach(f=>{
      const st = festifsDone[f.id];
      html += `<div class="card pop-in dossier">
        <span class="badge dossier-tag">🥂 FACULTATIF</span>
        ${st ? `<span class="badge ${st==='done'?'done':''}">${st==='done'?'✅ Fait !':'⏭️ Passé'}</span>` : ""}
        <h3>${f.titre}</h3>
        <p>${f.description}</p>
        ${!st ? `<div class="grid-2">
            <button class="btn-block btn-outline btn-sm" data-festif-skip="${f.id}">Passer</button>
            <button class="btn-block btn-sm" data-festif-done="${f.id}">✔️ Fait !</button>
          </div>` : ""}
      </div>`;
    });
  }
  ...
  // Le Coffre IPC — finale commune, visible une fois les 5 équipes
  ...
  if (allQuizDone && !(config && config.coffreOpened)){
    html += `
      <div class="card pulse" style="border-color:var(--accent); text-align:center; margin-top:22px;">
        ...
        <input type="text" id="coffre-input" placeholder="Code complet" style="text-align:center;font-size:1.3rem;letter-spacing:4px;text-transform:uppercase;margin-top:8px;">
        <button class="btn-block" id="coffre-submit-btn" style="margin-top:8px;">🔓 Ouvrir le coffre</button>
        <p class="dim" id="coffre-status" style="margin-top:6px;"></p>
      </div>`;
  }
  ...
  document.querySelectorAll("[data-festif-done]").forEach(b=> b.onclick = async ()=>{
    await Store.markFestif(teamId, b.dataset.festifDone, "done");
    toast("🥂 Bien joué !", "", "success");
  });
  document.querySelectorAll("[data-festif-skip]").forEach(b=> b.onclick = async ()=>{
    await Store.markFestif(teamId, b.dataset.festifSkip, "skipped");
  });

  const coffreBtn = $("coffre-submit-btn");
  if (coffreBtn) coffreBtn.onclick = async ()=>{
    const input = $("coffre-input");
    const status = $("coffre-status");
    const code = (input && input.value) || "";
    coffreBtn.disabled = true; coffreBtn.textContent = "Vérification...";
    const res = await Store.validateCoffreCode(code);
    if (res.ok){
      ...
      coffreBtn.disabled = false; coffreBtn.textContent = "🔓 Ouvrir le coffre";
