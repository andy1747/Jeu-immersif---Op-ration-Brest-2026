...
const AudioFX = (function(){
  ...
  let introNodes = [];        // nappe de suspense jouée pendant la cinématique d'intro
  ...
  /* ---------------- MUSIQUE DE SUSPENSE (cinématique d'intro) ----------------
     Nappe tendue façon "espionnage" : drone grave filtré + pointes aiguës
     aléatoires façon compte à rebours. Jouée pendant l'écran de chargement et
     la cinématique terminal, puis coupée au profit de l'ambiance d'équipe
     normale dès que la mission démarre (voir stopIntroSting côté app.js). */
  function startIntroSting(){
    stopIntroSting();
    const c = ensureCtx();
    const drone = c.createOscillator(); const drone2 = c.createOscillator();
    const droneGain = c.createGain();
    drone.type = "sawtooth"; drone.frequency.value = 55;
    drone2.type = "sine"; drone2.frequency.value = 110;
    const filt = c.createBiquadFilter(); filt.type = "lowpass"; filt.frequency.value = 260;
    droneGain.gain.value = 0.055;
    drone.connect(filt); drone2.connect(filt); filt.connect(droneGain); droneGain.connect(masterGain);
    drone.start(); drone2.start();
    introNodes.push(drone, drone2);
    const id = setInterval(()=>{
      if (Math.random() < 0.45) tone(1100 + Math.random()*500, 0.1, "square", 0, 0.045);
      else if (Math.random() < 0.3) noiseBurst(0.04, 3200, 0, 0.045);
    }, 780);
    introIntervalIds.push(id);
  }
  function stopIntroSting(){
    introNodes.forEach(n=>{ try{ n.stop(); }catch(e){} });
    introNodes = [];
    introIntervalIds.forEach(id=> clearInterval(id));
    introIntervalIds = [];
  }

  return { ensureCtx, startAmbient, stopAmbient, isMuted, setMuted, toggleMuted, play, startIntroSting, stopIntroSting };
})();
