...
function renderScoresTab(c){
  ...
  const rows = TEAM_ORDER.map(tid=>{
    ...
    return `<div class="card">
      ...
      <p class="dim">${done}/${total||"?"} missions · ${d.powerUsed?"pouvoir utilisé":"pouvoir disponible"} ${blocked?" · ⛔ bloquée":""}${team.dormant?` · 🕶️ ${d.handlerScore||0} pts Handler`:""}</p>
      <div class="grid-2">
...
function renderDormantsTab(c){
  ...
  const finalDef = GAME_DATA.finalMissions["final-potter"];
  const potterData = TEAMS_CACHE.potter || {};
  const omegaDone = !!(potterData.completed && potterData.completed["final-potter"] && potterData.completed["final-potter"].status === "done");
  ...
  c.innerHTML = `
    ...
    <div class="card" style="border-color:var(--danger);">
      <h3>⚠️ Protocole Omega</h3>
      <p class="dim">Code à communiquer par « Roland » en personne : <b style="letter-spacing:3px;">${finalDef.omegaCode}</b></p>
      <p class="dim">${omegaDone ? "✅ Code déjà validé par Harry Potter." : "En attente de saisie du code par Harry Potter."}</p>
    </div>
    <div class="card" style="text-align:center;">
      <h3>🏁 Révélation finale</h3>
      <p class="dim">Déclenche l'écran de fin (classement, réussite/échec du Protocole Omega, révélation des agents dormants) chez toutes les équipes en même temps.</p>
      <button class="btn-block btn-danger" id="reveal-btn" ${CONFIG_CACHE.finaleRevealed ? "disabled" : ""}>
        ${CONFIG_CACHE.finaleRevealed ? "✅ Révélation déjà déclenchée" : "🏁 Déclencher la révélation finale"}
      </button>
    </div>
  `;
