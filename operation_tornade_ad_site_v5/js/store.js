...
function defaultTeam(){
  return { score:0, unlockedCount:1, completed:{}, powerUsed:false, powerUsedAt:null,
           ...
           drawnChallenges: {}, festifsDone: {} };
}

/* ---------------- TIRAGE DES DÉFIS IPC (chapitre 2) ----------------
   Choisit "count" défis pour une équipe : 1 défi "produit" (quiz1)
   garanti + le reste au hasard parmi les autres catégories (jamais les
   défis festifs, jamais deux fois le même pour une même équipe). Comme
   pour le Quiz IPC, se rabat automatiquement sur d'autres catégories si
   une catégorie est vide (ex: "culture" tant qu'elle n'est pas remplie). */
function selectChapterDraw(count){
...
const Store = {
    ...
    if (LOCAL_MODE){
      ...
      teamIds.forEach(tid => {
        ...
        if (slot && typeof slot === "object" && slot.draw){
          ...
          if (!t.drawnChallenges[chapterNum]) t.drawnChallenges[chapterNum] = selectChapterDraw(slot.draw);
        }
      });
      if (chapterNum >= 3){
        teamIds.forEach(tid => {
    ...
    for (const tid of teamIds){
      ...
      if (slot && typeof slot === "object" && slot.draw){
        ...
        if (!drawnChallenges[chapterNum]){
          drawnChallenges[chapterNum] = selectChapterDraw(slot.draw);
          hasDraw = true;
          drawBatch.set(teamRef, { drawnChallenges }, { merge:true });
        }
      }
    }
  ...
  async answerSingleQuiz(teamId, missionId, selectedIndex){
    ...
    if (LOCAL_MODE){
      const d = loadLocalDB();
      const t = d.teams[teamId] = d.teams[teamId] || defaultTeam();
      if (t.completed && t.completed[missionId] && t.completed[missionId].status==="done") return { ok:true, already:true, correct: t.completed[missionId].correct };
      t.completed = t.completed || {};
    ...
    await db.runTransaction(async (tx) => {
      ...
      if (team.completed && team.completed[missionId] && team.completed[missionId].status==="done"){
        out = { ok:true, already:true, correct: team.completed[missionId].correct };
        return;
      }
  ...
  // Marque un défi festif (facultatif) comme "fait" ou "passé" — jamais de
  // points, juste un petit suivi pour ne pas re-proposer le même en boucle.
  async markFestif(teamId, festifId, status){
    if (LOCAL_MODE){
      const d = loadLocalDB();
      const t = d.teams[teamId] = d.teams[teamId] || defaultTeam();
      t.festifsDone = t.festifsDone || {};
      t.festifsDone[festifId] = status; // "done" | "skipped"
      saveLocalDB(d);
      return;
    }
    const teamRef = db.collection("teams").doc(teamId);
    await db.runTransaction(async (tx) => {
      const snap = await tx.get(teamRef);
      const team = snap.exists ? snap.data() : defaultTeam();
      const festifsDone = Object.assign({}, team.festifsDone, { [festifId]: status });
      tx.set(teamRef, { festifsDone }, { merge:true });
    });
  },
